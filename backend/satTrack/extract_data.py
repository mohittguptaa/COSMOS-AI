...
def find_culmination_times(tle_data, latitude, longitude, altitude_degrees=0.0):
    # Create Skyfield EarthSatellite object
    lines = tle_data.strip().splitlines()
    satellite = EarthSatellite(lines[0], lines[1], 'SAT', load.timescale())

    # Load timescale and set up location
    ts = load.timescale()
    topos = wgs84.latlon(float(latitude), float(longitude))

    # Define time period for the search (5 days from now)
    t0 = ts.now()
    t1 = ts.now() + timedelta(days=5)

    # Find the satellite events
    times, events = satellite.find_events(topos, t0, t1, altitude_degrees=altitude_degrees)

    # Filter for culmination events (event type 1)
    culmination_times = [times[i] for i in range(len(events)) if events[i] == 1]

    return culmination_times
